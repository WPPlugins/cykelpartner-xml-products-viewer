...
function CPxmlProductsViewer($data) {
	$html = '';
	...
	if($network == 'pa') {
		...
		$affiliate_id = get_option('cp_td');
		$url = 'http://pf.tradedoubler.com/pf/pf?a='.$affiliate_id.'&mintdprice=0.0&maxtdprice=1000000.0&recurse=true&programs=210647&productName='.$query[0].'&description=&maxResults=5000&firstResult=0&xmlVersion=2';
		//echo $url;
		$fields = array(
			'products' => 'products',
			'product' => 'product',
			'productsid' => 'TDProductId',
			'productsname' => 'name',
			'productsdescription' => 'description',
			'productsprice' => 'price',
			'productsurl' => 'productUrl',
			'productsimageurl' => 'imageUrl',
			'categoryname' => 'TDCategories',
			'brand' => 'brand',
			'currency' => 'currency'
		);
	}/* elseif($network == 'wg') {
		...
		echo 'Fejl: Du skal vælge affiliate netværk. Brug network="NETWORK" variablen.';
		die();
	}
	if(empty($affiliate_id)) {
		echo 'Fejl: Du skal ind på indstillingssiden og indtaste dit partner id eller site id.';
		die();
	}
	$template = !empty($data['template']) ? $data['template'] : 'template.tpl';
	$template_content = file_get_contents(dirname(__FILE__).'/'.$template);
	
	$cache_timeout = get_option('cp_cache_timeout');
	...
	if($network == 'td') {
		$filename = CACHEDIR.'cache-'.$network.'-q-'.filename($query_filename).'-e-'.filename($exclude_filename).'.xml';
	} else {
		$filename = CACHEDIR.'cache-'.$network.'.xml';
	}
	$cache = request_cache($url, $filename, $cache_timeout);
	/*if($td_cache_timeout > 0) {
		chmod(CACHEDIR.$filename,0755);
	}*/
	if($cache === false) {
	...
	for($x = 0; ($x < sizeof($products) && $y < $limit); $x++) {
		...
		if(!empty($query[0])) {
			for($i = 0; $i < count($query) && $pass === false; $i++) {
				if(strpos(strtolower($categoryname), $query[$i]) !== false || strpos(strtolower($products[$x][$fields['productsname']]), strtolower($query[$i])) !== false) {
					$p++;
				}
			}
			if(count($query) == $p) {
		...
		if(!empty($exclude[0])) {
			for($i = 0; $i < count($exclude) && $pass === true; $i++) {
				if(strpos(strtolower($products[$x][$fields['productsname']]), strtolower($exclude[$i])) !== false) {
					$pass = false;
				}
			}
		}
		...
		if($pass === true) {			
			$html .= str_replace(array(
				...
				array(
					...
					$template_content);
			$y++;
			if($y%$newlineafter == 0) $html .= '<div style="clear:both;"></div>';
		}
	}
	//echo '<hr />'.$x;
